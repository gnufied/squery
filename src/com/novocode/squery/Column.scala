...
trait ConvertableColumn[T] extends Column[T] {
...
abstract case class ConstColumn[T](val value: T) extends ConvertableColumn[T] { self: SimpleColumn[T] =>
...
abstract class WrappedColumn[T](val parent: SimpleColumn[T]) extends ConvertableColumn[T] { self: SimpleColumn[T] =>
  def dumpThis(out: PrintWriter, prefix: String, name: String, dumper: Dump.Dumper) {
...
abstract class Table[T](val tableName: String) extends TableBase[T] with ConvertableColumn[T] { self =>
...
sealed trait Projection[T <: Product] extends ConvertableColumn[T] with Product {
  ...
  def setParameter(ps: PositionedParameters, value: T): Unit = {
    for(i <- 0 until productArity) {
      ...
      productElement(i).asInstanceOf[ConvertableColumn[Any]].setParameter(ps, v)
    }
  }
}
